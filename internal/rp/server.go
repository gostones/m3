...
//var susage string = `frps is the server of frp
//
//Usage:
...
func Server(opt string) {
	var err error
	//confFile := "./frps.ini"
	// the configures parsed from file will be replaced by those from command line if exist
	//args, err := docopt.Parse(susage, []string{}, true, version.Full(), false)

	//if args["-c"] != nil {
	//	confFile = args["-c"].(string)
	//}

	in := strings.NewReader(opt)
	...
	config.ServerCommonCfg.ConfigFile = ""
	config.ServerCommonCfg.PrivilegeMode = false
	config.ServerCommonCfg.TcpMux = true
